...
import {
    ...
    parseStickerPack,
    stickerContent,
} from "../../../../src/utils/beeper/imagePacks";
...
function fakeClient(rooms: Room[], accountData: Record<string, object> = {}): MatrixClient & {
    roomState: jest.Mock;
    getStateEvent: jest.Mock;
} {
    return {
        ...
        getStateEvent: jest.fn().mockRejectedValue(new Error("M_NOT_FOUND")),
    } as unknown as MatrixClient & { roomState: jest.Mock; getStateEvent: jest.Mock };
}
...
describe("MSC2545 sticker packs", () => {
    ...
    it("collects packs from the room, user_emotes, emote_rooms and joined spaces, deduplicated", async () => {
        ...
        const listed = fakeRoom("!listed:x", { a: TELEGRAM_PACK, b: { ...TELEGRAM_PACK, pack: { display_name: "B" } } });
