...
import { effectiveImageSize, isOneToOneRoom, isTelegramLayout } from "../../../../src/utils/beeper/telegramLayout";
...
describe("Telegram-style layout", () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(SettingsStore, "getValue").mockImplementation(((name: string, ...rest: any[]) => {
    ...
    describe("isOneToOneRoom", () => {
        ...
        it("treats two-member rooms and bridged DMs as one-to-one, but not groups", () => {
            ...
            expect(isOneToOneRoom(makeRoom(["@me:example.org", BOT, "@telegram_1:example.org"], "group"))).toBe(
                false,
            );
