...
export interface EventTileProps {
    ...
    /** Whether this event is ticked in message selection mode. */
    isSelected?: boolean;
    /** Whether the timeline is in message selection mode (bulk delete/forward). */
    isSelecting?: boolean;
...
export class UnwrappedEventTile extends React.Component<EventTileProps, IState> {
    ...
    private selectionRoot: HTMLElement | null = null;

    private bindSelectionListeners(): void {
        const root = this.props.forExport || this.isPreview ? null : this.ref.current;
        if (root === this.selectionRoot) return;
        this.unbindSelectionListeners();
        root?.addEventListener("click", this.onSelectionToggle);
    ...
    private readonly onRootMouseDown = (e: globalThis.MouseEvent): void => {
        // Stop shift-click (range selection) from selecting text.
        if (e.shiftKey) e.preventDefault();
    };

    private readonly onSelectionToggle = (e: globalThis.MouseEvent): void => {
    ...
    private readonly onPermalinkClicked = (e: MouseEvent): void => {
        ...
        e.preventDefault();
        // In selection mode a click toggles the selection instead (see onSelectionToggle).
        if (this.props.isSelecting) return;
        MessageSelectionStore.instance.setAnchorId(this.props.mxEvent.getRoomId()!, this.props.mxEvent.getId()!);
        dis.dispatch<ViewRoomPayload>({
            action: Action.ViewRoom,
            event_id: this.props.mxEvent.getId(),
            highlighted: true,
    ...
    private createViewModelProps(inputs: EventTileRenderInputs = this.createRenderInputs()): EventTileViewModelProps {
        ...
        return {
            ...
            display: {
                ...
                isSelected:
                    this.props.isSelectedEvent || !!this.state.interaction.contextMenu || !!this.props.isSelected,
            ...
            sender: {
                member: this.withPerMessageProfile(
    ...
    private withPerMessageProfile(member: MemberInfo | null): MemberInfo | null {
        const displayName = getPerMessageProfile(this.props.mxEvent)?.displayname;
    ...
    public render(): ReactNode {
        ...
        // Info messages (joins, …) keep their small avatar.
        const hideAvatar =
            this.props.hideAvatar &&
            eventTileSnapshot.sender.profileState.needsSenderProfile &&
            this.context.timelineRenderingType === TimelineRenderingType.Room;
        const avatar =
            !hideAvatar && avatarMember && eventTileSnapshot.sender.profileState.avatarSize !== null ? (
                <PerMessageProfileAvatar
                    mxEvent={this.props.mxEvent}
                    size={eventTileSnapshot.sender.profileState.avatarSize}
                >
        ...
        return (
            <EventTileView
                ...
                line={{
                    ...
                    sticker: eventTileRenderState.line.sticker || isAnimatedSticker(this.props.mxEvent),
                }}
                classNames={{
