...
import { editMessage, sendMessage } from "./message";
import defaultDispatcher from "../../../../../dispatcher/dispatcher";
import SettingsStore from "../../../../../settings/SettingsStore";
import { SettingLevel } from "../../../../../settings/SettingLevel";
import EditorStateTransfer from "../../../../../utils/EditorStateTransfer";
import * as ConfirmRedactDialog from "../../../dialogs/ConfirmRedactDialog";
...
describe("message", () => {
    ...
    mockClient.setDisplayName = vi.fn().mockResolvedValue({});
    mockClient.setRoomName = vi.fn().mockResolvedValue({});
    ...
    describe("sendMessage", () => {
        ...
        describe("slash commands", () => {
            const getCommandSpy = vi.spyOn(SlashCommands, "getCommand");
            ...
            it("returns undefined when the command is not successful", async () => {
                // When
                const validCommand = "/spoiler";
                vi.spyOn(Commands, "runSlashCommand").mockResolvedValueOnce([
            ...
            it.each(messagesAndEffectCategoryTestCases)(
                ...
                async (inputText) => {
                    ...
                    expect(mockClient.sendMessage).toHaveBeenCalledWith(
                        "myfakeroom",
                        null,
                        expect.not.objectContaining({ "m.relates_to": expect.any(Object) }),
                    );
                },
            );
            ...
            it("calls addReplyToMessageContent when there is an event to reply to", async () => {
                const addReplySpy = vi.spyOn(Reply, "addReplyToMessageContent");
            ...
            it("if user enters invalid command and then sends it anyway", async () => {
                ...
                vi.spyOn(Commands, "shouldSendAnyway").mockResolvedValueOnce(true);
            ...
            it("if user enters invalid command and then does not send, return undefined", async () => {
                ...
                vi.spyOn(Commands, "shouldSendAnyway").mockResolvedValueOnce(false);
                const invalidCommandInput = "/badCommand";
    ...
    describe("editMessage", () => {
        ...
        it("Should cancel editing and ask for event removal when message is empty", async () => {
            // When
            const mockCreateRedactEventDialog = vi.spyOn(ConfirmRedactDialog, "createRedactEventDialog");
