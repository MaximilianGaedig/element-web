...
import {
    ...
    retryFailedMessage,
} from "../../../utils/bridge/messageSendStatus";
...
function failureText(status: MessageSendStatus): string {
    const network = status.network || _t("bridge|send_status_remote_network");
    if (status.message) return _t("bridge|send_status_failed_with_message", { network, message: status.message });
    switch (status.reason) {
        case "com.beeper.unsupported_event":
            return _t("bridge|send_status_failed_unsupported", { network });
        case "m.event_too_old":
            return _t("bridge|send_status_failed_too_old", { network });
        case "m.no_permission":
            return _t("bridge|send_status_failed_no_permission", { network });
        case "m.bridge_unavailable":
            return _t("bridge|send_status_failed_bridge_unavailable", { network });
        default:
            return _t("bridge|send_status_failed", { network });
    }
}
 ...
 */
export default function MessageSendStatus({ mxEvent, failuresOnly }: Props): JSX.Element | null {
    const client = useContext(MatrixClientContext);
    ...
    if (status.status === "PENDING") {
        return (
            <Tooltip label={status.message ?? _t("bridge|send_status_pending", { network })}>
                {/* Focusable so keyboard users can reach the tooltip. */}
                {/* oxlint-disable-next-line jsx-a11y/no-noninteractive-tabindex */}
                <span className="mx_SendStatus mx_SendStatus_pending" role="status" tabIndex={0}>
    ...
    if (status.status === "SUCCESS") {
        ...
        return (
            <Tooltip label={_t("bridge|send_status_delivered", { network })}>
                {/* oxlint-disable-next-line jsx-a11y/no-noninteractive-tabindex */}
                <span className="mx_SendStatus mx_SendStatus_delivered" role="status" tabIndex={0}>
    ...
    return (
        <div className="mx_SendStatus mx_SendStatus_failed" role="status">
            <ErrorSolidIcon width="16" height="16" aria-hidden />
            <span className="mx_SendStatus_text">{failureText(status)}</span>
            {retriable && (
                <AccessibleButton
                    kind="link_inline"
                    className="mx_SendStatus_retry"
                    onClick={onRetry}
                    disabled={retrying}
                >
                    <RestartIcon width="14" height="14" aria-hidden />
                    {retrying ? _t("bridge|send_status_retrying") : _t("bridge|send_status_retry")}
                </AccessibleButton>
            )}
        </div>
    );
