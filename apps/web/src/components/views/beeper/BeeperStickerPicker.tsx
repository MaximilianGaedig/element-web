...
import { loadStickerPacks, type PackImage, type StickerPack, stickerContent } from "../../../utils/beeper/imagePacks";
...
function StickerThumb({ image, onSend }: { image: PackImage; onSend: (image: PackImage) => void }): JSX.Element {
    ...
    if (inView) {
        if (isVideo) {
            ...
            preview = (
                <video
                    className="mx_BeeperStickerPicker_media"
            ...
            preview = <img className="mx_BeeperStickerPicker_media" src={src ?? undefined} alt="" loading="lazy" />;
        }
    }
    ...
    return (
        <AccessibleButton
            ref={ref}
            element="button"
            className="mx_BeeperStickerPicker_sticker"
            title={image.body}
...
function PackTab({
    ...
    return (
        <AccessibleButton
            ...
            className={classNames("mx_BeeperStickerPicker_tab", { mx_BeeperStickerPicker_tab_selected: selected })}
            title={pack.name}
...
export function BeeperStickerPickerPanel({
    ...
    if (!packs) {
        body = (
            <div className="mx_BeeperStickerPicker_status">
                <Spinner />
                {_t("beeper|sticker_picker_loading")}
            </div>
        );
    } else if (!images.length) {
        body = <div className="mx_BeeperStickerPicker_status">{_t("beeper|sticker_picker_empty")}</div>;
    } else {
        body = (
            <div className="mx_BeeperStickerPicker_grid" role="tabpanel" aria-label={q ? undefined : selected?.name}>
                {images.map((image) => (
    ...
    return (
        <div className="mx_BeeperStickerPicker" style={{ width: PICKER_WIDTH, height: PICKER_HEIGHT }}>
            <input
                className="mx_BeeperStickerPicker_search"
                type="search"
                placeholder={_t("beeper|sticker_picker_search")}
                aria-label={_t("beeper|sticker_picker_search")}
                value={query}
 ...
 */
export default function BeeperStickerPicker(props: Props): JSX.Element {
    const client = useMatrixClientContext();
        ...
        <>
            ...
            {isStickerPickerOpen && !useElementPicker && (
                <ContextMenu
                    ...
                    {...props.menuPosition}
                >
                    <BeeperStickerPickerPanel packs={current} onSend={onSend} />
                </ContextMenu>
            )}
        </>
    );
