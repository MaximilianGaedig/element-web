...
/**
 * Shows the avatar from a Beeper per-message profile (bridge relay mode) instead of the sender's.
...
export default function PerMessageProfileAvatar({ mxEvent, size, children }: Props): JSX.Element {
    ...
    return (
        <BaseAvatar
            ...
            url={url}
            title={_t("beeper|per_message_profile_via", { name, sender: mxEvent.getSender() ?? "" })}
