...
describe("MessageContextMenu", () => {
    ...
    describe("message pinning", () => {
        ...
        it("pins event on pin option click", async () => {
            ...
            vi.spyOn(room, "getAccountData").mockReturnValue(pinsAccountData);
    ...
    describe("quote button", () => {
        ...
        it("shows quote button when selection is inside one MTextBody and getSelectedText returns text", () => {
            vi.mocked(getSelectedText).mockReturnValue("quoted text");
            const isSelectionWithinSingleTextBody = vi
        ...
        it("dispatches ComposerInsert with quoted text when quote button is clicked", () => {
            vi.mocked(getSelectedText).mockReturnValue("line1\nline2");
            const dispatchSpy = vi.spyOn(dispatcher, "dispatch");
            const isSelectionWithinSingleTextBody = vi
                .spyOn(MessageContextMenu.prototype as any, "isSelectionWithinSingleTextBody")
                .mockReturnValue(true);
    ...
    describe("isSelectionWithinSingleTextBody", () => {
        let mockGetSelection: MockInstance;
        let contextMenuInstance: MessageContextMenu;

        beforeEach(() => {
            vi.clearAllMocks();

            mockGetSelection = vi.spyOn(window, "getSelection");
    ...
    describe("right click", () => {
        it("copy button does work as expected", () => {
            ...
            vi.mocked(getSelectedText).mockReturnValue(text);
        ...
        it("does not show edit button when we cannot edit", () => {
            const eventContent = createMessageEventContent("hello");
            vi.mocked(canEditContent).mockReturnValue(false);
...
function createMenu(
    ...
    client.getUserId = vi.fn().mockReturnValue("@user:example.com");
    client.getRoom = vi.fn().mockReturnValue(room);

    return render(
        <ScopedRoomContextProvider {...(context as RoomContextType)}>
            <MessageContextMenu mxEvent={mxEvent} onFinished={vi.fn()} {...props} />
