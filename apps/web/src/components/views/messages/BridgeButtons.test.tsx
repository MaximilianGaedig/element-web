...
import { parseBridgeButtons, parseBridgeKeyboard } from "../../../utils/BridgeButtons";

vi.mock("../../../utils/strings", async () => ({
    ...(await vi.importActual<object>("../../../utils/strings")),
    copyPlaintext: vi.fn(),
}));
...
describe("<BridgeButtons />", () => {
    ...
    it("renders nothing for redacted events even if the field is present", () => {
        ...
        vi.spyOn(ev, "isRedacted").mockReturnValue(true);
    ...
    describe("button types", () => {
        ...
        it("copies copy_text to the clipboard and says so", async () => {
            const user = userEvent.setup();
            vi.mocked(copyPlaintext).mockResolvedValue(true);
        ...
        it.each([
            ...
            const user = userEvent.setup();
            const spy = vi.spyOn(dis, "dispatch");
        ...
        it.each([
            ...
            expect(vi.mocked(client.sendTextMessage).mock.calls).toEqual(
                calls ? [[roomId, null, "!tg click 7 0 0"]] : [],
            );
        });
