...
export function formatLastSeen(
    ...
    if (presence === "online") return _t("beeper|last_seen_online");
    if (typeof statusMsg !== "string" || !statusMsg.startsWith(PREFIX)) return undefined;
    const rest = statusMsg.slice(PREFIX.length).trim();
    switch (rest) {
        case "recently":
            return _t("beeper|last_seen_recently");
        case "within a week":
            return _t("beeper|last_seen_within_week");
        case "within a month":
            return _t("beeper|last_seen_within_month");
    }
