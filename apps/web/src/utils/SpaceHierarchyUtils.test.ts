...
import { getSpacePath } from "./SpaceHierarchyUtils";
...
describe("SpaceHierarchyUtils", () => {
    ...
    describe("getSpacePath()", () => {
        ...
        beforeEach(() => {
            ...
            vi.spyOn(SDKContextClass.instance.spaceStore, "getParents").mockImplementation((roomId) => {
                if (roomId === room.roomId) return [space2];
                if (roomId === space2.roomId) return [space1];
                return [];
            });

            vi.spyOn(SDKContextClass.instance.spaceStore, "getCanonicalParent").mockReturnValue(null);
        });
